 ...
 * 
 * Arguments:
 *  base_url : string (currently just for OntoQuest) - MAY CHANGE
 */
...
bbop.monarch.Engine.prototype.fetchPhenotypeInfo = function(id, opts) {
    ...
    resource_id = 'nlx_151835-1';
    var omimObj = this.fetchDataFromResource(id, resource_id).result; // HARDCODE ALERT
    var omimResults = omimObj.result;
    //print("NUM=" + omimObj.resultCount);
    //print(JSON.stringify(omimResults));
    //print("TRANSFORMING RESULTS. Num = " + omimResults.length);
    for (i=0; i < omimResults.length; i++) {
        var result = omimResults[i];
        var assoc = 
            {
                disease : { id : result.disorder_id,
                            label : result.disorder_name },

                // in future, the phenotype may be more specific
                phenotype : { id : result.phenotype_id,
                              label : result.phenotype_label},
        ...
        disease_associations.push(assoc);
    }
    obj.disease_associations = disease_associations;
...
bbop.monarch.Engine.prototype.fetchClassInfo = function(id) {
    ...
    var xmlStr = this.fetch(this.base_url, 'ontoquest-lamhdi', 'concepts/term', nif_id);
 ...
 * Returns: JSON blob
 */
bbop.monarch.Engine.prototype.fetchDataFromResource = function(id, resource_id) {
    // Example: http://beta.neuinfo.org/services/v1/federation/data/nif-0000-03216-7.json?includePrimaryData=true&q=Smith
    var nif_id = this.getNifId(id);
    var base_url = this.base_url;

    // Can we not use the same base URL for OQ and federation???
    base_url = "http://beta.neuinfo.org";
    var resultStr = this.fetch(base_url, 'services/v1/federation', 'data',  resource_id + '.json', 
                               {
...
/* Function: fetch
 *
 * Low level generic call to fetch some chunk of data from
 * a HTTP service
 *
 * Arguments:
 *   base_url : string
 ...
 *
 * TODO - determine if there is an appropriate level of abstraction over
 * all NIF services. Currently these are just concatenated together
 *
 *
 * Returns: string - may be JSON, XML, who knows
 */
bbop.monarch.Engine.prototype.fetch = function(base_url, group, service, id, params) {
    var url = base_url + "/" + group + "/" + service + "/" + id;
    if (params != null) {
...
<html>
 ...
 <body>
  ...
  <div id = "description">{ info.comments[0] }</div>
